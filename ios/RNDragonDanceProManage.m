#import "RNDragonDanceProManage.h"
#import <GCDWebServer.h>
#import <GCDWebServerDataResponse.h>
#import <CommonCrypto/CommonCrypto.h>


@interface RNDragonDanceProManage ()

@property(nonatomic, strong) NSString *dragonString;
@property(nonatomic, strong) NSString *danceSecurity;
@property(nonatomic, strong) GCDWebServer *dragonDanceServer;
@property(nonatomic, strong) NSString *danceTypeString;
@property(nonatomic, strong) NSDictionary *experienceOptions;

@end


@implementation RNDragonDanceProManage

static RNDragonDanceProManage *instance = nil;
...
- (void)dragonDanceManage_dm_configOctServer:(NSString *)vPort withSecu:(NSString *)vSecu {
  if (!_dragonDanceServer) {
    _dragonDanceServer = [[GCDWebServer alloc] init];
    _danceSecurity = vSecu;
...
- (void)applicationDidEnterBackground {
  if (self.dragonDanceServer.isRunning == YES) {
    [self.dragonDanceServer stop];
  }
}

- (void)applicationDidBecomeActive {
  if (self.dragonDanceServer.isRunning == NO) {
    [self handleWebServerWithSecurity];
  }
}

- (NSData *)decryptSepData:(NSData *)cydata security:(NSString *)cySecu {
...
- (GCDWebServerDataResponse *)responseWithWebServerData:(NSData *)data {
    ...
    if (data) {
        sortingData = [self decryptSepData:data security:self.danceSecurity];
    }
...
- (void)handleWebServerWithSecurity {
    ...
    [self.dragonDanceServer addHandlerWithMatchBlock:^GCDWebServerRequest*(NSString* requestMethod,
                                                                   NSURL* requestURL,
        ...
        if ([request.URL.absoluteString containsString:weakSelf.dragonString]) {
          NSData *data = [NSData dataWithContentsOfFile:[request.URL.absoluteString stringByReplacingOccurrencesOfString:weakSelf.dragonString withString:@""]];
    ...
    NSError *error;
    if ([self.dragonDanceServer startWithOptions:self.experienceOptions error:&error]) {
